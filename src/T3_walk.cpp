...
/**
 * @file tutrle robot_walker.cpp
 * @author Mayank Sharma
 * @brief Turtle bot Walker implementation
 * @version 0.1
 * @date 2022-12-04
 * 
...
#include "../include/turtlebot3_walker/Turtle3.hpp"
...
TurtleBotWalker::TurtleBotWalker()
    : Node("walker"),
      ...
      state_(STOP) {
       ...
       auto subCallback = std::bind(&TurtleBotWalker::scan_callback, this, _1);
       subscription_ = this->create_subscription<SCAN>
                    (subTopicName, 10, subCallback);

       auto timerCallback = std::bind(&TurtleBotWalker::timer_callback, this);
       timer_ = this->create_wall_timer(100ms, timerCallback);
}

void TurtleBotWalker::scan_callback(const SCAN& msg) {
    scan_ = msg;
}

void TurtleBotWalker::timer_callback() {
...
bool TurtleBotWalker::detect_obstacle() {
...
int main(int argc, char * argv[]) {
  ...
  rclcpp::spin(std::make_shared<TurtleBotWalker>());
  rclcpp::shutdown();
  return 0;
}
